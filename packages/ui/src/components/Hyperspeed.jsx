...
/**
 * @param {{ effectOptions?: typeof DEFAULT_OPTIONS; debug?: boolean }} props
 */
const Hyperspeed = ({ effectOptions = DEFAULT_OPTIONS, debug = false }) => {
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      setDebugInfo(info => {
        if (info.width === width && info.height === height && info.hasCanvas === hasCanvas) {
          return info;
        }
        return { ...info, width, height, hasCanvas };
      });
    };
  ...
  useEffect(() => {
    ...
    class App {
      constructor(container, options = {}) {
        ...
        try {
          ...
          setDebugInfo(info => ({ ...info, status: 'renderer-error' }));
          throw error;
        }
        ...
        setDebugInfo(info => ({ ...info, status: 'renderer-ready' }));
    ...
    (function () {
      ...
      if (container.offsetWidth === 0 || container.offsetHeight === 0) {
        setDebugInfo(info => ({ ...info, status: 'waiting-for-size' }));
        const waitForSize = () => {
          ...
          requestAnimationFrame(() => myApp.onWindowResize());
          setDebugInfo(info => ({ ...info, status: 'running' }));
        };
        rafId = requestAnimationFrame(waitForSize);
        return;
      }
  ...
  return (
    ...
    >
      {showDebug && (
        ...
        >
          <div>Hyperspeed: {debugInfo.status}</div>
          <div>
            size: {debugInfo.width}x{debugInfo.height}
          </div>
          <div>canvas: {debugInfo.hasCanvas ? 'yes' : 'no'}</div>
        </div>
      )}
    </div>
  );
